function setupGraphiQLHelpers(fetchURL, socketURL){
    var token = null;
    var socket = io(socketURL);
    var inputsInWhichToSetAuthToken = null;

    var toastContainer = document.createElement("div");
    toastContainer.className = "toast-container docs";
    ...
    function pushToast(data) {
        ...
        toast.querySelector("strong").innerHTML = data.title;
    ...
    function setupGraphiQL() {
        ...
        var targets = document.getElementsByClassName('graphiql');
    ...
    function updateTokenInputs(token) {
        if (!inputsInWhichToSetAuthToken) {
            inputsInWhichToSetAuthToken = getInputsInWhichToSetAuthToken();
        }
        for (let i = 0; i < inputsInWhichToSetAuthToken.length; i++) {
            var input = inputsInWhichToSetAuthToken[i];
    ...
    function setRandomCredentials() {
        var username = randomUserName();
        var targets = document.getElementsByClassName('graphiql');
        for (let i = 0; i < targets.length; i++) {
            var target = targets[i];
            var query = target.getElementsByClassName("query")[0].innerHTML.trim();
            target.getElementsByClassName("query")[0].innerHTML = query.replace(/yourname/g, username);
        }
    }
    ...
    function randomUserName() {
        ...
        var name = left[Math.floor(Math.random() * left.length)] + "-" + right[Math.floor(Math.random() * left.length)];
