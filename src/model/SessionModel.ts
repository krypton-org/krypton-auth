...
/** @see {@link ISessionModel#removeOutdatedSessions} */
SessionSchema.statics.removeAllOutdatedSessions = async function(): Promise<any> {
    return await this.deleteMany({ expiryDate: { $lt: new Date() } });
};

/** @see {@link ISessionModel#removeSession} */
SessionSchema.statics.updateSession = async function(
    userId: string,
    refreshToken: string,
): Promise<{ refreshToken: string; expiryDate: Date }> {
    const data = {
...
SessionSchema.statics.getUserAndSessionFromRefreshToken = async function(
    refreshToken: string,
