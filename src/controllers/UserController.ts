...
const ensureLoggedIn = (req: any): void => {
...
exports.checkEmailAvailable = async function (email: string): Promise<{ isAvailable: boolean }> {
    const emailExists = await User.userExists({ email })
    return { isAvailable: !emailExists };
};

exports.checkUsernameAvailable = async function (username: string) {
    const usernameExists = await User.userExists({ username })
    return { isAvailable: !usernameExists };
};

exports.confirmEmail = async function (req: any, res: any, next: any): Promise<void | never> {
    ...
    try {
        ...
        ejs.renderFile(serviceConfig.notificationPageTemplate, { notifications }, { notifications }, function (err, html) {
            if (err) {
                next(err);
            } else {
                res.send(html);
...
exports.createUser = async function (user: any, req: any): Promise<{ user: any, notifications: Notification[] } | never> {
    const notifications: Notification[] = [];
...
exports.resendConfirmationEmail = async function (req: any): Promise<{ notifications: Notification[] } | never> {
    ensureLoggedIn(req);
    const notifications: Notification[] = [];
    ...
    if (user.verified) {
        ...
        sendConfirmationEmail(req.user, user.verificationToken, serviceConfig.host ? serviceConfig.host : req.headers.host)
...
exports.recoverPassword = async function (password: string, passwordRecoveryToken: string): Promise<{ notifications: Notification[] } | never> {
    let notifications = [];
...
exports.updateUser = async function (userUpdates: any, req: any, res: any): Promise<{ user: any, notifications: Notification[] } | never> {
    ensureLoggedIn(req);
    let notifications = [];
    ...
    try {
        ...
        if (!isEmailVerified) {
            sendConfirmationEmail(req.user, userUpdates.verificationToken, serviceConfig.host ? serviceConfig.host : req.headers.host);
            notifications.push({ type: 'info', message: 'You will receive a confirmation link at your email address in a few minutes.' });
        }
        return {
            notifications,
...
exports.deleteUser = async function (password: string, req: any): Promise<{ notifications: Notification[] } | never> {
    ensureLoggedIn(req);
    const notifications: Notification[] = [];
...
const sendConfirmationEmail = (user: any, confirmationToken: string, host: string): void => {
    agenda.now('email', {
        email: user.email,
        template: serviceConfig.verifyEmailTemplate,
        locals: {
...
exports.login = async function (login: string, password: string, res: any): Promise<{ token: string, user: any } | never> {
    ...
    if (emailExists)
        payload = await User.sign({ email: login }, password, serviceConfig.privateKey);
    else if (usernameExists)
        payload = await User.sign({ username: login }, password, serviceConfig.privateKey);
    else {
        res.status(401);
    ...
    if ((!refreshToken) ||
        ...
        refreshTokenExpiryDate.setTime(refreshTokenExpiryDate.getTime() + serviceConfig.refreshTokenExpiryTime);
...
exports.sendPasswordRecoveryEmail = async function (email: string, req: any): Promise<{ notifications: Notification[] } | never> {
    const notifications: Notification[] = [];
    ...
    const host = serviceConfig.host ? serviceConfig.host : req.headers.host;
    agenda.now('email', {
        locals: {
            link: "http://" + host + "/form/reset/password?token=" + passwordRecoveryToken,
            user
        },
        template: serviceConfig.resetPasswordEmailTemplate,
        email: email,
...
exports.resetPasswordForm = function (req: any, res: any, next: any): void {
    const notifications: Notification[] = [];
    if (req.user) {
    ...
    ejs.renderFile(serviceConfig.resetPasswordFormTemplate, locals, {}, function (err, html) {
        if (err) {
            next(err);
        } else {
            res.send(html);
...
exports.refreshToken = async function (req: any, res: any): Promise<{ token: string; expiryDate: Date;} | never> {
    ...
    if (user && user.refreshTokenExpiryDate && now.getTime() < user.refreshTokenExpiryDate.getTime()) {
        const payload = await User.refreshAuthToken({ _id: user._id }, serviceConfig.privateKey);
