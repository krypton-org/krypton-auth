...
const generateToken = function (tokenLength, cb) {
    return crypto.randomBytes(tokenLength, cb).toString("hex");
};
...
exports.updateUser = async function (userUpdates, req, res) {
    ensureLoggedIn(req);
    let notifications = [];
...
exports.login = async function (login, password, res) {
    let payload;
...
exports.refreshToken = async function (req, res) {
    ...
    if (user && user.refreshTokenExpiryDate && now.getTime() < user.refreshTokenExpiryDate.getTime()) {
        payload = await User.refreshAuthToken({ _id: user._id }, config.privateKey);
